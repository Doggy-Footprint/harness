...
2. Create the tracking file: Create a file at `agent-docs/synced-comments/<synced_id>.md` with the following structure. `code_hash` fingerprints the participating files' non-comment content (each file's content with comments stripped, concatenated in alphanumeric order of filename, hashed):
   ````
   ---
   version: 1
...
DO NOT arbitrarily determine unspecified details of task. Freely talk back to resolve undetermined and ambiguous details.
