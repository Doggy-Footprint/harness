...
4. **Amend (main).** Wait until both have reported. For each challenge: amend the contract, or reject it with a reason. Never let a subagent resolve a gap. After an amendment, continue each existing subagent (Claude: `SendMessage`; Codex: its follow-up tool). Send a complete replacement instruction containing the current contract path and version, the applicable Version Log entry, and the remaining assignment. It supersedes the earlier instruction; do not send only information to append. Spawn a fresh `implementer` or `test-implementer` only when the original cannot be continued.
